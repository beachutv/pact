...
export default function DashboardPage() {
  ...
  useLocationUpdate(user.id, 'dashboard')
  ...
  // Re-fetch circle members to get latest live locations
  useEffect(() => {
  ...
  // Init: check calendar connection + auto-sync
  useEffect(() => {
    ...
    async function init() {
      ...
      if (conn && !hasAutoSynced.current) {
        ...
        try {
          ...
          if (!res.ok) console.error('Auto-sync failed:', res.status, await res.text())
        } catch (e) {
  ...
  // Date param handling (from Spots tab)
  ...
  useEffect(() => {
    ...
    if (!isNaN(y) && !isNaN(m)) { setViewYear(y); setViewMonth(m) }
  ...
  const sparks = useMemo((): Spark[] => {
    ...
    return result.sort((a, b) => a.travelTime - b.travelTime).slice(0, 3)
  ...
  async function toggleManualHour(date: string, hour: number) {
    ...
    if (busy) {
      await supabase.from('busy_blocks').delete()
        ...
        .lte('start_hour', hour).gt('end_hour', hour)
      ...
      const { data } = await supabase.from('busy_blocks')
        ...
        .select('user_id, date, start_hour, end_hour').single()
  ...
  function changeMonth(delta: number) {
    ...
    if (m < 0) { m = 11; y-- }
    if (m > 11) { m = 0; y++ }
    setViewMonth(m); setViewYear(y); setSheetDate(null)
  }
  ...
  function renderDays() {
    ...
    for (let d = 1; d <= dim; d++) {
      ...
      if (hasConfirmed) bg = 'rgba(245, 158, 11, 0.18)'
      if (hasPending) { bg = 'rgba(59,130,246,0.15)'; borderColor = '#3b82f6' }
  ...
  const upcomingPacts = pacts.slice(0, 3)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0, position: 'relative' }}>
      <div
        ref={pullRef}
        {...touchHandlers}
        style={{ padding: '14px 16px 90px', overflowY: 'auto', flex: 1 }}
      >
        ...
        {/* ====== TOP SECTION: Pacts + Sparks ====== */}

        {/* Spark cards (auto-computed, system-triggered) */}
              ...
              return (
                ...
                >
                  ...
                  <p style={{ fontSize: 13, color: 'var(--text2)', marginTop: 2 }}>
                    {p.win_start !== null ? fmtHour(p.win_start) : '?'} – {p.win_end !== null ? fmtHour(p.win_end) : '?'} · {count}
                  </p>
        ...
        {connected === false && (
          <div className="card" style={{ textAlign: 'center', marginBottom: 14 }}>
            ...
            <p style={{ fontSize: 14, fontWeight: 700, marginBottom: 4 }}>Connect your calendar</p>
            <p style={{ fontSize: 12, color: 'var(--text2)' }}>
              See when everyone in {activeCircle.name} is free. We only check busy/free — never see event details.
            </p>
        ...
        {connected && (
          <>
            {/* Syncing indicator */}
            {syncing && (
              <div style={{ textAlign: 'center', fontSize: 11, color: 'var(--text2)', marginBottom: 8 }}>
